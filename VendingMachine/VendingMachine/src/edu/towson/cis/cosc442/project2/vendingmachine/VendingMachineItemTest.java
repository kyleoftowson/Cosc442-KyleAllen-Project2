...
public class VendingMachineItemTest {
	...
	public void testVendingMachineItem() {
		assertTrue(item.getName().equals("chips")&&item4.getPrice()==89);
		try{
	...
	public void testGetName() {
		...
		assertTrue(item.getName().equals("cookies"));
		assertTrue(item.getName().equals("1"));
		assertTrue(item.getName().equals(""));
		assertTrue(item.getName().equals("1.0"));
	}

	@Test
	public void testGetPriceWhenException() {
		VendingMachineItem item9=new VendingMachineItem("",0);
		try{
		...
		try{
		double response=item9.getPrice();
